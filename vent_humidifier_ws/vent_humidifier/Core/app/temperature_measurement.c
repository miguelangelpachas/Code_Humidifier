...
void Chamber_Temperature_Measure(void){
	...
	if(HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY) == HAL_OK)
	{
		...
		if((v_adc_volt < TEMP_PROBE_PRESENCE_DETECT_UPPER_THRESHOLD_V) && (v_adc_volt > TEMP_PROBE_PRESENCE_DETECT_LOWER_THRESHOLD_V)){
			__disable_irq();
		...
		if((alarm_flags & (1 << CHAMBER_PROBE_CONN_ALARM_FLAG)) || (alarm_flags &(1 << AIRWAY_PROBE_CONN_ALARM_FLAG))){
			...
			alarm_flags &= ~(1 << DISPLAY_LOW_TEMP_ALARM_FLAG);
			...
			if(system_flags & (1 << NON_HEATER_MODE_FLAG)){
				...
				if(system_flags & (1 << INVASIVE_MODE_FLAG)){
					...
					if(chamber_temperature_c < INV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_LOWER_THRESHOLD_C){
						__disable_irq();
						alarm_flags |= (1 << CHAMBER_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else if(chamber_temperature_c > INV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_UPPER_THRESHOLD_C){
						__disable_irq();
						alarm_flags &= ~(1 << CHAMBER_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else{
					...
					if(chamber_temperature_c < NINV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_LOWER_THRESHOLD_C){
						__disable_irq();
						alarm_flags |= (1 << CHAMBER_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else if(chamber_temperature_c > NINV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_UPPER_THRESHOLD_C){
						__disable_irq();
						alarm_flags &= ~(1 << CHAMBER_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else{
		...
		if((alarm_flags & (1 << CHAMBER_PROBE_CONN_ALARM_FLAG)) || (alarm_flags & (1 << AIRWAY_PROBE_CONN_ALARM_FLAG))){
			...
			if(system_flags & (1 << NON_HEATER_MODE_FLAG)){
				...
				if(system_flags & (1 << INVASIVE_MODE_FLAG)){
					...
					if(airway_temperature_c < INV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_LOWER_THRESHOLD_C){
						__disable_irq();
						alarm_flags |= (1 << AIRWAY_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else if(airway_temperature_c > INV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_UPPER_THRESHOLD_C){
						__disable_irq();
						alarm_flags &= ~(1 << AIRWAY_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else{
					...
					if(airway_temperature_c < NINV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_LOWER_THRESHOLD_C){
						__disable_irq();
						alarm_flags |= (1 << AIRWAY_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else if(airway_temperature_c > NINV_PROBE_PLACEMENT_PRE_WARM_UP_TEMP_UPPER_THRESHOLD_C){
						__disable_irq();
						alarm_flags &= ~(1 << AIRWAY_PROBE_MISPLACED_ALARM_FLAG);
						__enable_irq();
					}else{
